...
/// 
/// `result` is an array of size `SHA256-RESULT_SIZE`, which is 32 bytes. This call cannot fail.
...
/// `data` is a reference to an array of arbitrary length. 
/// 
/// `result` is an array of size `SHA384-RESULT_SIZE`, which is 48 bytes. This call cannot fail.
