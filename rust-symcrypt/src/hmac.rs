...
impl HmacSha256State {
    ...
    fn result(&mut self) -> [u8; SHA256_HMAC_RESULT_SIZE] {
        let mut result = [0u8; SHA256_HMAC_RESULT_SIZE];
        unsafe {
            symcrypt_sys::SymCryptHmacSha256Result(&mut self.state, result.as_mut_ptr());
        }
...
pub fn hmac256(key: &[u8], data: &[u8]) -> [u8; SHA256_HMAC_RESULT_SIZE] {
    let mut result = [0; SHA256_HMAC_RESULT_SIZE];
    unsafe {
...
pub fn hmac384(key: &[u8], data: &[u8]) -> [u8; SHA384_HMAC_RESULT_SIZE] {
    let mut result = [0u8; SHA384_HMAC_RESULT_SIZE];
